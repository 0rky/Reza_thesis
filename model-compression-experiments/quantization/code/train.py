...
SYSTEM_INFO_PATH = os.path.join(OUTPUT_DIR, "quantization_system_info.json")
EPOCH_PROFILE_PATH = os.path.join(OUTPUT_DIR, "quantization_epoch_profile.csv")
RUNTIME_SUMMARY_PATH = os.path.join(OUTPUT_DIR, "quantization_runtime_summary.txt")
LAYER_PROFILE_FLOAT_PATH = os.path.join(OUTPUT_DIR, "quantization_float_layer_profile.csv")
LAYER_PROFILE_DYNAMIC_PATH = os.path.join(OUTPUT_DIR, "quantization_dynamic_layer_profile.csv")
...
if __name__ == "__main__":
    ...
    profile_model_layers(model, device, sample_batch, LAYER_PROFILE_FLOAT_PATH)
    print(f"Saved layer-wise profile to: {LAYER_PROFILE_FLOAT_PATH}")
    ...
    profile_model_layers(quantized_model, torch.device("cpu"), sample_batch_cpu, LAYER_PROFILE_DYNAMIC_PATH)
    print(f"Saved layer-wise profile to: {LAYER_PROFILE_DYNAMIC_PATH}")
    ...
    with open(RUNTIME_SUMMARY_PATH, "w") as f:
        f.write("===== QUANTIZATION RUNTIME / SYSTEM SUMMARY =====\n")
        f.write(f"Total parameters: {total_params}\n")
        f.write(f"Trainable parameters: {trainable_params}\n")
        f.write(f"Optimizer state size (bytes): {optimizer_state_size_bytes}\n")

        float_state_size_bytes = get_file_size_bytes(float_state_path)
        float_onnx_size_bytes = get_file_size_bytes(onnx_path)
        dynamic_state_size_bytes = get_file_size_bytes(quantized_state_path)
        if float_state_size_bytes is not None:
            f.write(f"Float state dict size (bytes): {float_state_size_bytes}\n")
            f.write(f"Float state dict size (MB): {float_state_size_bytes / (1024 * 1024):.6f}\n")
        if float_onnx_size_bytes is not None:
            f.write(f"Float ONNX size (bytes): {float_onnx_size_bytes}\n")
            f.write(f"Float ONNX size (MB): {float_onnx_size_bytes / (1024 * 1024):.6f}\n")
        if dynamic_state_size_bytes is not None:
            f.write(f"Dynamic state dict size (bytes): {dynamic_state_size_bytes}\n")
            f.write(f"Dynamic state dict size (MB): {dynamic_state_size_bytes / (1024 * 1024):.6f}\n")
