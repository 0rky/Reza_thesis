...
def evaluate_with_metrics(model, device, test_loader, class_names, prefix):
    ...
    print(f"Saved confusion matrix to structured_pruning/outputs/{prefix}_confusion_matrix.png")
...
def compute_zero_weight_sparsity(model):
    total_zeros = 0
    total_params = 0
...
def compute_structured_channel_sparsity(model):
    total_structured_units = 0
    zero_structured_units = 0

    for module in model.modules():
        if isinstance(module, nn.Conv2d):
            w = module.weight.detach().cpu()
            out_channels = w.shape[0]
            reshaped = w.view(out_channels, -1)
            norms = torch.norm(reshaped, p=2, dim=1)
            zero_structured_units += torch.sum(norms == 0).item()
            total_structured_units += out_channels
            ...
            zero_structured_units += torch.sum(norms == 0).item()
            total_structured_units += out_features

    structured_sparsity = 100.0 * zero_structured_units / total_structured_units if total_structured_units > 0 else 0.0
...
if __name__ == "__main__":
    ...
    onnx_path = os.path.join(CHECKPOINT_DIR, "structured_dense.onnx")
    ...
    for module in prunable_modules:
        prune.ln_structured(
            module,
            name="weight",
            amount=STRUCTURED_PRUNING_AMOUNT,
            n=2,
            dim=0
        )

    zeros_before_remove, total_before_remove, zero_sparsity_before_remove = compute_zero_weight_sparsity(model)
    zero_units_before_remove, total_units_before_remove, structured_sparsity_before_remove = compute_structured_channel_sparsity(model)

    print(f"Zero-weight sparsity before remove (%): {zero_sparsity_before_remove:.2f}")
    print(f"Structured sparsity before remove (%): {structured_sparsity_before_remove:.2f}")
    ...
    print(f"Zero-weight sparsity after remove (%): {zero_sparsity_after_remove:.2f}")
    print(f"Structured sparsity after remove (%): {structured_sparsity_after_remove:.2f}")

    structured_state_path = os.path.join(CHECKPOINT_DIR, "structured_pruned_state_dict.pth")
    torch.save(model.state_dict(), structured_state_path)
    print(f"Saved: {structured_state_path}")

    dense_size_mb = get_file_size_mb(dense_state_path)
    structured_size_mb = get_file_size_mb(structured_state_path)
    reduction_pct = 100.0 * (dense_size_mb - structured_size_mb) / dense_size_mb if dense_size_mb > 0 else 0.0

    print("\n===== STRUCTURED PRUNING SIZE / SPARSITY COMPARISON =====")
    print(f"Dense model size (MB): {dense_size_mb:.4f}")
    print(f"Structured pruned model size (MB): {structured_size_mb:.4f}")
    print(f"Size reduction (%): {reduction_pct:.2f}")
    print(f"Zero-weight sparsity after pruning (%): {zero_sparsity_after_remove:.2f}")
    print(f"Structured sparsity after pruning (%): {structured_sparsity_after_remove:.2f}")

    with open(os.path.join(OUTPUT_DIR, "structured_size_sparsity_comparison.txt"), "w") as f:
        f.write("===== STRUCTURED PRUNING SIZE / SPARSITY COMPARISON =====\n")
        f.write(f"Dense model size (MB): {dense_size_mb:.6f}\n")
        f.write(f"Structured pruned model size (MB): {structured_size_mb:.6f}\n")
        f.write(f"Size reduction (%): {reduction_pct:.6f}\n")
        f.write(f"Zero-weight sparsity after pruning (%): {zero_sparsity_after_remove:.6f}\n")
        f.write(f"Structured sparsity after pruning (%): {structured_sparsity_after_remove:.6f}\n")
        f.write(f"Zero weights after pruning: {zeros_after_remove}\n")
        f.write(f"Total prunable weights: {total_after_remove}\n")
        f.write(f"Zero structured units after pruning: {zero_units_after_remove}\n")
